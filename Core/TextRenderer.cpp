...
namespace TextRenderer
{
    ...
    public:
        ...
        void LoadFromBinary( const wchar_t* fontName, const Utility::StorageType* pBinary, const size_t binarySize )
        {
            (fontName);
            ...
            SetName( m_Texture.GetResource(), "Font-Texture" );
    ...
    const Font* GetOrLoadFont(const wstring& filename)
    {
        ...
        if (filename == L"default")
            newFont->LoadFromBinary(L"default", (Utility::StorageType*)g_pconsola24, sizeof(g_pconsola24));
        else
...
void TextRenderer::Initialize( void )
{
	...
	const InputDesc vertElem[] =
    {
        ...
        { "TEXCOORD", 0, DXGI_FORMAT_R16G16B16A16_UINT, 0, 8, D3D11_INPUT_PER_INSTANCE_DATA, 1 }
    };
    ...
    s_TextPSO[0].SetPrimitiveTopologyType(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
    s_TextPSO[0].SetVertexShader( MY_SHADER_ARGS(g_pTextVS) );
    s_TextPSO[0].SetPixelShader( MY_SHADER_ARGS(g_pTextAntialiasPS) );
    ...
    s_ShadowPSO[0].SetPixelShader( MY_SHADER_ARGS(g_pTextShadowPS) );
...
void TextContext::SetRenderState( void )
{
    ...
    {
        m_Context.SetDynamicConstantBufferView(0, sizeof(m_VSParams), &m_VSParams, { kBindVertex } );
        m_VSConstantBufferIsStale = false;
    }

    if (m_PSConstantBufferIsStale)
    {
        m_Context.SetDynamicConstantBufferView(0, sizeof(m_PSParams), &m_PSParams, { kBindPixel } );
        m_PSConstantBufferIsStale = false;
    }

    if (m_TextureIsStale)
    {
        m_Context.SetDynamicDescriptor(0, m_CurrentFont->GetTexture().GetSRV(), { kBindPixel } );
...
void TextContext::DrawString( const std::wstring& str )
{
    ...
    if (primCount > 0)
    {
        m_Context.SetDynamicSampler(0, SamplerLinearClamp, { kBindPixel });
