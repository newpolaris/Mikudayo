...
PixelShaderOutput main(PixelShaderInput input)
{
    ...
    if (useNormalMap) {
        float3x3 tangentFrame = ComputeTangentFrame(normal, input.eyeWS, input.texCoord);
        normal = mul(2.0f * tex2D(SubTexSampler, IN.Tex) - 1, tangentFrame);
        if (any(normal)) normal = normalize(normal);
        specularMapVal = tex2D(SubTexSampler, IN.Tex).a;
    }
    float3 Ln = normalize( -LightDirection );
    float3 Nn = normal;

    float rimPower = max(0, dot(Vn, -LightDirection));
    float NV = dot(Nn, Vn);
    ...
    if (!any(saturate(shadowCoord.xy) != shadowCoord.xy))
    {
        ...
        shadow = GetShadow( input.shadowPositionCS, input.positionCS.xyz );
        float lightIntensity = dot( normal, -SunDirectionWS );
        shadow = min( lightIntensity, shadow );
    }

    float ToonShade = smoothstep(SHADING_MIN, SHADING_MAX, HLambert);
    float comp = lerp( ToonShade, shadow*ToonShade, ShadowStrength*(1 - shadow) );
    ...
    float3 Specular = max(0, F*D*G/NV)*LightAmbient*shadow*SPECULAR_EXTENT;
